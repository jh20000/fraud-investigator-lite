...
st.markdown(
    """
    <div class="hero">
        <div class="section-label">0. 서비스 소개</div>
        <div class="eyebrow">JB금융그룹 Fin:AI Challenge · FraudOps MVP</div>
        <h1>FraudInvestigator Lite</h1>
        <p>기존 FDS 또는 Rule/ML 기반 1차 탐지 이후, 조사자가 의심거래의 근거 확인·Review 판단·고객 확인 질문·조사 리포트 초안을 빠르게 검토하도록 돕는 이상거래 조사 지원 AI Agent MVP입니다.</p>
        <div class="guide-grid">
            <div class="guide-item">자동 차단/자동 승인 아님</div>
            <div class="guide-item">최종 판단은 조사자 Human-in-the-loop</div>
            <div class="guide-item">예선 MVP는 Streamlit + 샘플 데이터 기반 시연</div>
            <div class="guide-item">실제 JB 내부 FDS/API/DB/규정 문서와 연동되어 있지 않음</div>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
st.markdown(
    ...
    <div class="workflow-card">
        <div class="section-label">1. Agent Workflow</div>
        <h2>6-Step Agent Workflow</h2>
        <div class="workflow-grid">
            <div class="step"><strong>1</strong><span class="step-body">거래 선택</span></div>
            <div class="step"><strong>2</strong><span class="step-body">거래·고객·가맹점 맥락 결합</span></div>
            <div class="step"><strong>3</strong><span class="step-body">Rule score / ML fraud score 확인</span></div>
            <div class="step"><strong>4</strong><span class="step-body">Decision Router: Danger / Review / Clean 분류</span></div>
            <div class="step"><strong>5</strong><span class="step-body">Review 거래 Evidence 검색</span></div>
            <div class="step"><strong>6</strong><span class="step-body">Review 고객 질문·조사 리포트 초안 생성</span></div>
        </div>
        <div class="workflow-note">Review 거래에 대해서만 Evidence 검색, 고객 확인 질문, 리포트 초안 생성을 수행합니다. Clean 거래는 정상 가능 사유만 표시합니다.</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with evidence_col:
    ...
    if decision == "Review":
        ...
        st.info("Review 거래가 아니므로 Evidence 검색과 고객 확인 질문 초안 생성을 수행하지 않습니다.")
        if decision == "Danger":
            st.write("**조사 우선순위 표시 근거**")
            for item in case["risk_evidence"]:
                st.write(f"- {item}")
        else:
            st.write("**Clean 분류 근거**")
            for item in case["normal_evidence"]:
                st.write(f"- {item}")
    st.markdown("</div>", unsafe_allow_html=True)
...
with report_col:
    st.markdown('<div class="panel">', unsafe_allow_html=True)
    st.markdown('<div class="panel-title">Review 리포트 초안</div>', unsafe_allow_html=True)
    if decision == "Review":
        st.text_area(
            "조사 리포트 초안",
        ...
        st.info("Review 거래가 아니므로 조사 리포트 초안 생성을 수행하지 않습니다.")
        if decision == "Danger":
            st.error("Danger 거래는 리포트 생성보다 조사 우선순위 표시와 즉시 확인 대상 식별에 초점을 둡니다.")
        else:
            st.success("Clean 거래는 정상 가능 사유를 표시하고 일반 모니터링 대상으로 정리합니다.")
    st.markdown("</div>", unsafe_allow_html=True)

st.caption(
    "FraudInvestigator Lite는 예선 MVP 시연용 프로토타입입니다. 조사자가 근거를 검토한 뒤 최종 판단을 확정합니다."
)
